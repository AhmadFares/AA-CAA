"""
Paper-ready AA figure: 1 row × 3 cols (MovieLens geo | CORDIS | MIMIC-IV).
Shows AA-Coverage evolution for all 5 variants, averaged over 40 URs.
No figure title (added manually in Overleaf).

Run from ~/TVD:
  .venv/bin/python3 scripts_server/plot_paper_aa_llm.py
"""
...
def plot_dataset(ax, ds, sqrt_scale=False):
    ...
    m, s = aggregate(sg, ur_ids, totals)
    if m is not None:
    ...
    m, s = aggregate(llmg, ur_ids, totals)
    if m is not None:
    ...
    m, s = aggregate(llma, ur_ids, totals)
    if m is not None:
    ...
    m, _ = aggregate(als, ur_ids, totals)
    if m is not None:
        ax.plot(GRID, m, color=COLOR_ALL, lw=1.8)

    ax.axhline(THETA, color="gray", lw=1, ls=":", alpha=0.5)
...
for sqrt_scale, suffix in [(False, ""), (True, "_sqrt")]:
    ...
    plt.tight_layout()
    out = OUTDIR / f"paper_aa_Final{suffix}.pdf"
