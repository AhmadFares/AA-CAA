"""
Paper-ready CAA figure with Prune step: 2 rows × 3 cols.
Same as paper_caa_Final but x-axis extended: 0%–100% trajectory + "Prune" point
showing average AV-Coverage (top) and Penalty (bottom) after pruning.

Run from ~/TVD:
  .venv/bin/python3 scripts_server/plot_paper_caa_with_prune.py
"""
...
DATASETS = [
    dict(
        name="MovieLens",
        step_files=["results/caa_auto_geo_v3/steps.csv"],
        sum_files =["results/caa_auto_geo_v3/summary.csv"],
        split="geo", lo=201, hi=240,
    ),
    dict(
        name="CORDIS",
        step_files=["results/caa_auto_cordis_v3/steps.csv"],
        sum_files =["results/caa_auto_cordis_v3/summary.csv"],
        split="candidates", lo=301, hi=340,
    ),
    dict(
        name="MIMIC-IV",
        step_files=["results/caa_auto_mimic_v3/steps.csv"],
        sum_files =["results/caa_auto_mimic_v3/summary.csv"],
...
def prune_val(df_sum, variant, final_col, lo, hi, split):
    sub = df_sum[(df_sum["variant"] == variant)
                 & df_sum["UR_id"].between(lo, hi)
                 & (df_sum["split"] == split)]
    if sub.empty:
        return None
    return sub[final_col].mean()
...
def plot_col(ax_top, ax_bot, ds, sqrt_scale=False):
    ...
    df_sum  = df_sum[df_sum["eps"] == EPS]

    for ax, metric, final_col, ylabel, better in [
        (ax_top, "ecoverage_current", "ecoverage_final", "AV-Coverage", "↑ better"),
        (ax_bot, "penalty_current",   "penalty_final",   "Penalty",     "↑ better"),
    ]:
        def plot_prune(variant, color, zorder=5):
            v = prune_val(df_sum, variant, final_col, ds["lo"], ds["hi"], ds["split"])
            if v is not None:
                ax.plot(X_PRUNE, v, marker="o", color=color,
                        markersize=10, zorder=zorder, clip_on=False)
...
for sqrt_scale, suffix in [(False, ""), (True, "_sqrt")]:
    fig, axes = plt.subplots(2, 3, figsize=(20, 8.0), sharey="row")
    ...
    plt.tight_layout()
    out = OUTDIR / f"paper_caa_with_prune{suffix}.pdf"
