...
class TestCases:
    ...
    def __init__(self):
        ...
        self.load_auto_movielens_geo_cases()  # Auto-generated geo URs (201-220)
        self.load_auto_cordis_cases()          # Auto-generated CORDIS URs (301-320)
        self.load_auto_mimic_cases()           # Auto-generated MIMIC URs (401-420)
    ...
    def load_auto_movielens_geo_cases(self):
        """
        Auto-generated URs for the MovieLens geo split, IDs 201-240.
        201-220 = auto_deep_1..20  (2-3 attrs, 3-5 values each)
        221-240 = auto_shallow_1..20 (4-5 attrs, 1-2 values each)

        Generated by helpers/auto_ur_generator.py from the geo split's
        stats.parquet + value_index.json.  Re-running generate_auto_urs()
        with the same seed=42 always yields the same URs.
        """
        import os
        try:
            from helpers.auto_ur_generator import generate_auto_urs
        except ImportError:
            # tolerate missing file gracefully
            return

        split_path = os.path.join(
            BASE_DIR, "data", "generated_splits", "MovieLens", "geo"
        )
        if not os.path.isdir(split_path):
            return  # split not available on this machine

        try:
            urs = generate_auto_urs(split_path, n_deep=20, n_shallow=20, seed=42)
        except Exception:
            return

        for i, name in enumerate(sorted(k for k in urs if k.startswith("auto_deep_"))):
            ur_id = 201 + i          # 201..220
            ur_df = self.create_flexible_dataframe(urs[name])
            self.cases[ur_id] = ur_df
            self.case_names[ur_id] = name

        for i, name in enumerate(sorted(k for k in urs if k.startswith("auto_shallow_"))):
            ur_id = 221 + i          # 221..240
            ur_df = self.create_flexible_dataframe(urs[name])
            self.cases[ur_id] = ur_df
            self.case_names[ur_id] = name

    def load_auto_cordis_cases(self):
        """
        Auto-generated URs for the CORDIS candidates split, IDs 301-340.
        301-320 = auto_deep_1..20
        321-340 = auto_shallow_1..20
        """
        import os
        try:
            from helpers.auto_ur_generator import generate_auto_urs
        ...
        split_path = os.path.join(
            BASE_DIR, "data", "generated_splits", "CORDIS", "candidates"
        )
        if not os.path.isdir(split_path):
            return
    ...
    def load_auto_mimic_cases(self):
        """
        Auto-generated URs for the MIMIC-IV admissions split, IDs 401-440.
        401-420 = auto_deep_1..20
        421-440 = auto_shallow_1..20
        Deep URs 1-6 use frac_upper=0.40; 7-20 fall back to frac_upper=0.50
        (handled automatically inside generate_auto_urs).
        """
        import os
        try:
            from helpers.auto_ur_generator import generate_auto_urs
        ...
        split_path = os.path.join(
            BASE_DIR, "data", "generated_splits", "MIMIC", "admissions"
        )
        if not os.path.isdir(split_path):
            return
